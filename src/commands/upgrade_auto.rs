...
pub fn analyse_interface(old_bytes: &[u8], new_bytes: &[u8]) -> InterfaceDiffReport {
    ...
    if old_hash == new_hash {
        non_breaking_issues.push("Old and new WASM are byte-identical; no interface change".to_string());
    }
...
fn handle_diff(args: DiffArgs) -> Result<()> {
    ...
    if let Some(out) = &args.out {
        fs::write(out, &rendered)
            .with_context(|| format!("Failed to write interface diff report to {}", out.display()))?;
...
mod tests {
    ...
    fn interface_diff_flags_removed_method_as_breaking() {
        let old = spec_wasm(
            vec![abi_function_entry(
                "transfer",
                vec![("to", ScSpecTypeDef::Address), ("amount", ScSpecTypeDef::I128)],
                ScSpecTypeDef::I128,
            )],
            b"",
        );
    ...
    fn interface_diff_flags_signature_change_as_breaking() {
        ...
        assert_ne!(report.changed_functions[0].before, report.changed_functions[0].after);
    }

    #[test]
    fn interface_diff_flags_type_removal_as_breaking() {
        let old = spec_wasm(
            vec![abi_struct_entry("Config", vec![("count", ScSpecTypeDef::U32)])],
            b"",
        );
        let new = spec_wasm(vec![], b"");
        let report = analyse_interface(&old, &new);
